...
class CRicerca {
    ...
    public function lista() {
        ...
        if (strlen($parametri[0]) > 3) {
            ...
            $pagine = ceil($num_risultati / $this->_annunci_per_pagina);
            if (isset($parametri['ordinamento']))
            ...
            $i = 0;
            foreach ($risultato as $ann) {
                $FAnnuncio = new FAnnuncio();
                $citta[$i] = $FAnnuncio->getCitta($ann->getCittaConsegna())[0];
                if ($citta[$i]['comune'] != $citta[$i]['provincia'])
                    $citta[$i] = $citta[$i]['comune'] . ' (' . $citta[$i]['provincia'] . ')';
                else
                    $citta[$i] = $citta[$i]['comune'];
                $i++;
            }
            $view->impostaDati('citta', $citta);
    ...
    public function creaAnnuncio() {
        ...
        if ($view->getCitta() != false)
            $annuncio['citta_consegna'] = $view->getCitta();
